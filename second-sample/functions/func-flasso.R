runlasso=function(Y,indice,lag,alpha=1,type="lasso"){
  ...
  model=biclasso(cbind(X,dum),y,alpha = alpha)
  coef=model$coef
  if(type=="adalasso"){
    penalty=(abs(coef[-1])+1/sqrt(length(y)))^(-1)
    model=biclasso(cbind(X,dum),y,penalty.factor = penalty,alpha=alpha)
  }
  ...
  if(type=="fal"){
    ...
    bb=Inf
    for(alpha in alphas){
      m0=biclasso(cbind(X,dum),y,alpha = alpha)
      coef=m0$coef
      for(tau in taus){
        penalty=(abs(coef[-1])+1/sqrt(length(y)))^(-tau)
        m=biclasso(cbind(X,dum),y,penalty.factor = penalty)
        crit=m$bic
        if(crit<bb){
          model=m
          bb=crit
...
lasso.rolling.window=function(Y,nprev,indice=1,lag=1,alpha=1,type="lasso"){
  ...
  save.coef=matrix(NA,nprev,4 + 2+ ncol(Y[,-1])*4 )
