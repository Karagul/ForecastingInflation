...
dum[which.min(Y[,1])]=1
Y=cbind(Y,dum=dum)
source("modelfunctions/functionsD/func-fact.R")
source("modelfunctions/functionsD/func-tfact.R")
...
##
write.table(cpi,"forecasts/presenteD/tfact-cpi.csv",sep=";",row.names = FALSE, col.names = FALSE)
write.table(pce,"forecasts/presenteD/tfact-pce.csv",sep=";",row.names = FALSE, col.names = FALSE)
