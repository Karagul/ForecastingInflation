runlasso=function(Y,indice,lag,alpha=1,type="lasso"){
  ...
  model=ic.glmnet(cbind(comb1,dum),y,alpha = alpha)
  coef=model$coef
  if(type=="adalasso"){
    penalty=(abs(coef[-1])+1/sqrt(length(y)))^(-1)
    model=ic.glmnet(cbind(comb1,dum),y,penalty.factor = penalty,alpha=alpha)
  }
  
  pred=predict(model,c(comb1out,0))
...
lasso.rolling.window=function(Y,nprev,indice=1,lag=1,alpha=1,type="lasso"){
  ...
  save.coef=matrix(NA,nprev,2+(20+ncol(Y[,-c(1:2)])*4)*126)
